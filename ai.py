...
class GroqClient:
    def __init__(self, model_id='meta-llama/llama-4-maverick-17b-128e-instruct')->None:
        self.model_id = model_id
        self.client = ChatGroq(model=self.model_id)
    
    def generete_response(self, prompt):
    ...
    def resume_cv(self, cv):
        prompt = f'''
            **Solicitação de Resumo de Currículo em Markdown:**
            ...
            Por favor, gere um resumo do currículo fornecido, formatado em Markdown, seguindo rigorosamente o modelo abaixo. **Não adicione seções extras, tabelas, linhas em branco desnecessárias ou qualquer outro tipo de formatação diferente da especificada.** Preencha cada seção com as informações relevantes, garantindo que o resumo seja preciso e focado. Para o nome, use exatamente o texto da primeira linha após `## Nome Completo` no currículo, sem modificações.
            SEMPRE RETORNE O RESULTADO EM MARKDOWN, SEM NENHUM TEXTO ADICIONAL ANTES OU DEPOIS DO RESULTADO.
        ...
        result_raw = self.generete_response(prompt=prompt)
